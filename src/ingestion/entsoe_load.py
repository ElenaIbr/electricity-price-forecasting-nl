# ================================
# Download HOURLY electricity load (NL)
...
OUTPUT_FILENAME = "nl_load_hourly_2019_2025.csv"
...
def main() -> None:
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        print(f"ENTSOE load: downloading {year}…")
        ...
        if isinstance(load, pd.Series):
            df_year = load.to_frame(name="load_mw")
        elif isinstance(load, pd.DataFrame):
            df_year = load.copy()
            df_year.columns = ["load_mw"]
        else:
