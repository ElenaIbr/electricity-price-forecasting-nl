...
def extract_wind_series(forecast: pd.DataFrame, debug: bool = False) -> pd.Series:
    ...
    if isinstance(forecast.columns, pd.MultiIndex):
        ...
        wind_cols = [
            ...
            if "wind" in top.lower()
        ]
        if not wind_cols:
            raise KeyError("Wind columns not found in MultiIndex forecast.")
        return forecast[wind_cols].sum(axis=1)

    wind_candidates = [
        col for col in forecast.columns
        if "wind" in str(col).lower()
    ]

    if not wind_candidates:
        raise KeyError(f"Wind columns not found. Available columns: {list(forecast.columns)}")

    return forecast[wind_candidates].sum(axis=1)
...
def main() -> None:
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        print(f"ENTSOE wind forecast: downloading {year}...")
        ...
        wind = extract_wind_series(forecast)
        df_year = wind.to_frame(name="wind_forecast_mw")
