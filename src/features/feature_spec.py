...
При изменении этого файла **обязательно** пересобрать и перепрогнать модель —
старые bundle'ы будут невалидны (это проверяется через feature_eng_hash).
"""
...
# Менять порядок — значит ломать совместимость bundle'ов.
# ──────────────────────────────────────────────────────────────────────────
